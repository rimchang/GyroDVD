...
class Encoder_shift_block(nn.Module):
    def __init__(self, n_features, kernel_size, reduction, bias=False, scale_unetfeats=48):
        ...
        # self.attn_weight1 = nn.Sequential(
        #     nn.AdaptiveAvgPool2d(1),
        #     nn.Conv2d(in_channels=n_feat + n_feat, out_channels=n_feat, kernel_size=1, stride=1, padding=0),
        #     nn.Sigmoid()
        # )
        #
    ...
    def forward(self, x, kernel):
        ...
        # attn = self.attn_weight1(torch.cat([x_ker, x], dim=1))
        # x_ker = x_ker * attn
...
class Encoder2(nn.Module):
    def __init__(self, n_features, kernel_size=3, reduction=4, bias=False, scale_unetfeats=48):
        ...
        self.decoder_level1_3 = Encoder_shift_block(n_feat, kernel_size, reduction, bias)
        self.decoder_level1_4 = Encoder_shift_block(n_feat, kernel_size, reduction, bias)
        self.decoder_level1_5 = Encoder_shift_block(n_feat, kernel_size, reduction, bias)
...
class GyroDVD(nn.Module):
    def __init__(self, n_feats2=128, future_frames=1, past_frames=1):
        ...
        self.feat_extract_ker = nn.Sequential(nn.Conv2d(16, self.n_feats0, 3, 1, 1))

        self.feat_extract_dK = nn.Sequential(nn.Conv2d(16, self.n_feats0, 3, 1, 1))
        ...
        self.orb1 = TFR_UNet(self.n_feats0, self.n_feats, kernel_size=3, reduction=4, act=nn.PReLU(), bias=False,
                             scale_unetfeats=0)
        self.orb1_ker = TFR_UNet(self.n_feats0, self.n_feats, kernel_size=3, reduction=4, act=nn.PReLU(), bias=False,
                             scale_unetfeats=0)
        self.orb1_dK = TFR_UNet(self.n_feats0, self.n_feats, kernel_size=3, reduction=4, act=nn.PReLU(), bias=False,
                             scale_unetfeats=0)
        self.conv_trans = conv(self.n_feats0, self.n_feats0, 3, bias=True)
        self.conv_trans_ker = conv(self.n_feats0, self.n_feats0, 3, bias=True)
        self.conv_trans_dK = conv(self.n_feats0, self.n_feats0, 3, bias=True)
    ...
    def stage0_ker(self, x0):
        shortcut = x0
        x0 = self.orb1_ker(x0)
        res0 = x0 + shortcut
        return res0, self.conv_trans_ker(res0)

    def stage0_dK(self, x0):
        shortcut = x0
        x0 = self.orb1_dK(x0)
        res0 = x0 + shortcut
        return res0, self.conv_trans_dK(res0)
    ...
    def forward(self, x, kernel=None, dK=None):
        ...
        x0_ker = self.feat_extract_ker(kernel)
        x0_dK = self.feat_extract_dK(dK)

        sam_features0, sam_features = self.stage0(x0)
        _, x0_ker = self.stage0_ker(x0_ker)
        _, x0_dK = self.stage0_dK(x0_dK)
        ...
        out_ker = kernel[self.num_fb:frames-self.num_ff]

        return out.unsqueeze(0), out_ker.unsqueeze(0)
